turtles-own [  ;; propriedades das tartarugas
...
patches-own [  ;; propriedades dos caminhos
...
to setup
  ...
  set-default-shape turtles "person" ;; padrão - pessoas
  make-turtles ;; cria as tartarugas
  identity
  ;; recolor
  ;; ask patches [ set p-infected? false ] ;; todos os caminhos não estão infectados
  ;; infect ;; função infect
  ;; recolor
...
to identity
  ...
  ask n-of A_supporters turtles [ ;; numero de infectados
    set A_party? true
    set color red
    set A_adoption support_threshold
  ]
  ask n-of B_supporters turtles with [not A_party?] [ ;; numero de infectados
    set B_party? true
    set color blue
    set B_adoption support_threshold
  ]
  ask n-of C_supporters turtles with [not A_party? and not B_party?] [ ;; numero de infectados
    set C_party? true
    set color yellow
    set C_adoption support_threshold
  ]
  ask n-of D_supporters turtles with [not A_party? and not B_party? and not C_party?] [ ;; numero de infectados
    set D_party? true
    set color green
...
;;to infect
  ;;ask n-of num-infected turtles [ ;; numero de infectados
    ;;set infected? true ;; infecta
    ;;if variant = "environmental" [
      ;; the patch under an infected turtle becomes infected:
      ;;set p-infected? true
...
to spread_opinion
  ask turtles [
    ;;let A_influence count (turtles-at -1 0) with [A_party?]
    ;;let max_influence 0
    ;;let min_influence 0
    ;;let max_ideology 0
    ...
    ;;let min_influence min (list A_influence B_influence C_influence D_influence)
    ...
    ;;let B_influence count (turtles-at -1 0) with [B_party?]
    ;;let C_influence count (turtles-at -1 0) with [C_party?]
    ;;let D_influence count (turtles-at -1 0) with [D_party?]
  ]
end
