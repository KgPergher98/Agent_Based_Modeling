...
to setup
  ...
  ;; create random network based on the chooser
  if network = "random" [
  nw:generate-random turtles links num-agents density [
    ;; adoption property needs to be initializes in the setup
    set adopted? false
    ;; separe the turtle spatially
    setxy random-xcor random-ycor
    ;; consistent appearence
    set color white
    set shape "person"
  ]
  ]
  ;; create the network based on preferential attachement
  ;; ignores the density slider
  if network = "preferential-attachement" [
    nw:generate-preferential-attachment turtles links num-agents density [
    ;; adoption property needs to be initializes in the setup
    set adopted? false
    ;; separe the turtle spatially
    setxy random-xcor random-ycor
  ...
  ask n-of (frac-influentials * num-agents) turtles [
...
to go
  ...
  ;; ask turtles to adopt or not adot randomly
...
;; reset diffusion without destroying the network
to reset-diffusion
  ask turtles [
    set adopted? false
    set color white
...
;; this procedure will determine whether or not adopt
to adopt
   ;; adopt based on bradcast influence
  if random-float 1.0 < broadcast-influence [
    set adopted? true
    set color red
  ]
  ;; adopt based on social-influence based on a network
  if ( breed = influentials )[
    ...
    if inf-total-neighbors > 0 [
      set influential-influence cinf-neighbors-adopted / inf-total-neighbors
    ]
    let regular-influence 0
    if reg-total-neighbors > 0 [
      set regular-influence reg-neighbors-adopted / reg-total-neighbors
    ]
...
network
network
"random" "preferencial-attachement"
0
